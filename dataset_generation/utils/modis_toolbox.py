...
Cloud2Street modis code.
Modified the following datasets due to deprecation:
 - MODIS TERRA/AQUA data
 - JRC/GSW1_1/YearlyHistory --> JRC/GSW1_4/YearlyHistory
 - USGS/GMTED2010 --> USGS/GMTED2010_FULL
 - JRC/GSW1_0/GlobalSurfaceWater --> JRC/GSW1_4/GlobalSurfaceWater
...
def get_jrc_perm(roi_bounds):
    jrc_perm_water = (
        ee.Image("JRC/GSW1_4/GlobalSurfaceWater").select("transition").eq(1).unmask()
    )
    return jrc_perm_water.select(["transition"], ["jrc_perm_water"]).clip(roi_bounds)


def get_land_mask(roi_bounds):
    """
    Returns a binary land mask (1 = land, 0 = ocean/elsewhere) using the
    USDOS/LSIB_SIMPLE/2017 land polygons, clipped to the input bounds.

    Parameters
    ----------
    roi_bounds : ee.Geometry
        Region of interest to clip the land mask.

    Returns
    -------
    ee.Image
        Binary land mask image with 1 over land and 0 elsewhere.
    """
    land_fc = ee.FeatureCollection("USDOS/LSIB_SIMPLE/2017").filterBounds(roi_bounds)
    land_mask = (
        ee.Image.constant(1)
        .clip(land_fc)
        .clip(roi_bounds)
...
def otsu_get_threshold(histogram):
...
def get_jrc_yearly_perm(began, roi):
    ...
    jrc_year = ee.Algorithms.If(
        ee_began.get("year").gt(2018), 2018, ee_began.get("year")
    )
    jrc_perm = (
        ee.Image(
            ee.ImageCollection("JRC/GSW1_4/YearlyHistory")
            .filterBounds(roi)
            .filterMetadata("year", "equals", jrc_year)
            .first()
        )
        .remap([0, 1, 2, 3], [0, 0, 0, 1])
        .unmask()
...
def dfo_bands_gq(collection):
    return collection.select(["sur_refl_b01", "sur_refl_b02"], ["red_250m", "nir_250m"])
...
def dfo_bands_ga(collection):
    return collection.select(
        ["sur_refl_b01", "sur_refl_b03", "sur_refl_b04", "sur_refl_b07", "state_1km"],
        ["red_500m", "blue", "green", "swir", "state_1km"],
    )
...
def get_aqua(roi, date_range):
    aqua_gq = (
        ee.ImageCollection("MODIS/061/MYD09GQ").filterDate(date_range).filterBounds(roi)
    )
    aqua_ga = (
        ee.ImageCollection("MODIS/061/MYD09GA").filterDate(date_range).filterBounds(roi)
    )
...
def get_terra(roi, date_range):
    terra_gq = (
        ee.ImageCollection("MODIS/061/MOD09GQ").filterDate(date_range).filterBounds(roi)
    )
    terra_ga = (
        ee.ImageCollection("MODIS/061/MOD09GA").filterDate(date_range).filterBounds(roi)
    )
    return ee.ImageCollection(
...
def pan_sharpen(image):
    ...
    return (
        image.select("red_250m", "nir_250m", "state_1km")
...
def cloud_calc(img):
    roi_pixels = ee.Number(
        img.select("state_1km")
        .reduceRegion(reducer=ee.Reducer.count(), maxPixels=1e9)
        .get("state_1km")
    )
    cloud_pixels = ee.Number(
        img.select("cloud_state")
...
def least_cloudy(img_coll):
    ...
    return ee.Image(
        non_null_images.filterMetadata("cloud_cover_perc", "equals", min_cloud).first()
    )
